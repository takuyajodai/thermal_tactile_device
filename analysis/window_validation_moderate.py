...
# -*- coding: utf-8 -*-
"""
Moderate window size validation module for Gaussian fitting analysis.

This module provides a moderate complexity validation method combining:
1. Fitting goodness of fit (primary indicator)
2. Statistical validation (Z-score, IQR method)
3. Theoretical validity (Peak-to-Baseline Ratio, auxiliary indicator)
4. Combined evaluation

This is a balanced approach, more comprehensive than simple but less complex than comprehensive.
"""
...
def validate_window_size_moderate(window_size: float, prob: float, B: float,
                                  all_windows: List[float],
                                  reduced_deviance: Optional[float] = None,
                                  reduced_chi2: Optional[float] = None) -> Dict[str, any]:
    """
    Moderate complexity validation of window size.
    
    This function performs a balanced validation using:
    1. Fitting goodness of fit (primary indicator)
    ...
    dict
        ...
        - Combined evaluation
        - All intermediate calculations (PBR, etc.)
    """
    result = {}
